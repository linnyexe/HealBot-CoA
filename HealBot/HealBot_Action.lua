...
function HealBot_Action_SetrSpell()
    ...
    -- CoA Custom Classes - universal range fallback
    else
        HealBot_hSpell=nil
        HealBot_bSpell=nil
        -- Try to find first usable spell from player's configured HealBot spells
        if HealBot_Globals and HealBot_Globals.Spell then
            for hk, hv in pairs(HealBot_Globals.Spell) do
                if HealBot_Globals.Spell[hk] then
                    for sk, sv in pairs(HealBot_Globals.Spell[hk]) do
                        local spName = HealBot_Globals.Spell[hk][sk]
                        if spName and spName ~= "" then
                            local spId = HealBot_GetSpellId(spName)
                            if spId then
                                HealBot_hSpell = spName
                                HealBot_bSpell = spName
                                break
                            end
                        end
                    end
                    if HealBot_hSpell then break end
                end
            end
        end
    end
...
-- CoA BG Flag Carrier - standalone event frame for UNIT_AURA
local hbFlagFrame = CreateFrame("Frame", "HealBot_CoA_FlagFrame")
hbFlagFrame:RegisterEvent("UNIT_AURA")
hbFlagFrame:SetScript("OnEvent", function(self, event, unit)
    if not HealBot_Unit_Bar1 then return end
    local bar = HealBot_Unit_Bar1[unit]
    if not bar then return end
    local hasFlag = nil
    for i=1,40 do
        local n = UnitAura(unit, i, "HELPFUL")
        if not n then break end
        if n == "Horde Flag" or n == "Alliance Flag" then hasFlag = true; break end
    end
    local flagIcon = _G[bar:GetName().."FlagIcon"]
    if hasFlag then
        if not flagIcon then
            flagIcon = bar:CreateTexture(bar:GetName().."FlagIcon", "OVERLAY")
            flagIcon:SetPoint("BOTTOMRIGHT", bar, "BOTTOMRIGHT", -3, 3)
            flagIcon:SetSize(16, 16)
            flagIcon:SetTexture([[Interface\Icons\INV_Banner_02]])
            flagIcon:SetTexCoord(0,1,0,1)
        end
        flagIcon:SetAlpha(1)
    elseif flagIcon then
        flagIcon:SetAlpha(0)
    end
end)
local ebusa=nil
...
--    if not uName then uName=HEALBOT_WORDS_UNKNOWN end
    ...
    ebuicon16 = _G[ebubar:GetName().."Icon16"];
    ...
    --bar.txt:SetTextHeight(btextheight);
        ...
        if Healbot_Config_Skins.HoTonBar[Healbot_Config_Skins.Current_Skin]==1 then
            ...
            if Healbot_Config_Skins.HoTposBar[Healbot_Config_Skins.Current_Skin]==1 then
                ...
                icon16:SetPoint("BOTTOMRIGHT", bar, "BOTTOMRIGHT", -3, 3);
                icon16:SetAlpha(0);
...
local hbClassCols = {
          -- Standard classes (unused in CoA but kept for init)
          ...
          -- CoA Custom Classes (from RAID_CLASS_COLORS)
          ["CHRO"] = {r=1.0,  g=0.93, b=0.29, },
          ["TINK"] = {r=0.85, g=0.85, b=0.85, },
          ["PROP"] = {r=0.42, g=0.65, b=0.0,  },
          ["RANG"] = {r=0.75, g=0.94, b=0.42, },
          ["NECR"] = {r=0.27, g=0.86, b=0.61, },
          ["WIND"] = {r=0.89, g=0.55, b=0.35, },
          ["CULT"] = {r=0.61, g=0.27, b=0.95, },
          ["GUAR"] = {r=0.61, g=0.58, b=0.51, },
          ["SUNC"] = {r=1.0,  g=0.7,  b=0.25, },
          ["MONK"] = {r=1.0,  g=0.98, b=0.7,  },
          ["BARB"] = {r=0.54, g=0.2,  b=0.01, },
          ["STOR"] = {r=0.0,  g=0.49, b=0.93, },
          ["STAR"] = {r=0.56, g=1.0,  b=1.0,  },
          ["REAP"] = {r=0.04, g=0.53, b=0.42, },
          ["SPIR"] = {r=0.25, g=0.78, b=0.92, },
          ["WITC"] = {r=0.96, g=0.0,  b=1.0,  },
          ["SONO"] = {r=0.64, g=0.0,  b=0.0,  },
          ["PYRO"] = {r=1.0,  g=0.38, b=0.07, },
          ["FLES"] = {r=0.99, g=0.0,  b=0.02, },
          ["DEMO"] = {r=0.46, g=0.98, b=0.0,  },
      }
